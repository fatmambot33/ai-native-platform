...
def _append_repository_findings(root: Path, findings: list[Finding]) -> None:
    ...
    if "ai-native validate" not in readme or "ai-native doctor" not in readme:
        findings.append(
            Finding(
                "standard.readme_cli_missing",
                "README must document the validation and doctor CLI.",
                "README.md",
            )
        )
...
def validate_standard(root: Path = ROOT) -> list[Finding]:
    """Return canonical repository conformance findings."""
...
def main(argv: list[str] | None = None) -> int:
    """Validate the canonical repository and emit deterministic output."""
    parser = argparse.ArgumentParser()
    parser.add_argument("--root", type=Path, default=ROOT)
    parser.add_argument("--json", action="store_true")
    args = parser.parse_args(argv)
    findings = validate_standard(args.root.resolve())
    if args.json:
